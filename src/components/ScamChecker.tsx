...
export const ScamChecker: React.FC<ScamCheckerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'text' | 'screenshot' | 'upload'>('text');

  // Input states
  const [scamText, setScamText] = useState<string>(initialPreset?.text || '');
  const [scamImage, setScamImage] = useState<string | null>(initialPreset?.imageUrl || null);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setIsPlayingAudio(seniorAudio.getIsSpeaking());
    }, 300);
  ...
  const quickQuestions = isZh
    ? [
        { key: 'q1', text: '这是诈骗还是官方信息？' },
        { key: 'q2', text: '如果我不理会它会被罚款吗？' },
        { key: 'q3', text: '怎么向官方机构求证真假？' },
      ]
    : [
        { key: 'q1', text: 'Is this a scam or official?' },
        { key: 'q2', text: 'Will I be fined if I ignore it?' },
        { key: 'q3', text: 'How do I verify with the agency?' },
      ];
  ...
  const handleImageFile = (file: File) => {
    ...
    reader.onload = () => {
      ...
      seniorAudio.speakText(
        isZh ? '图片已上传。您可以点击下方核查按钮。' : 'Image uploaded. Tap Check to verify.',
        language
      );
    };
  ...
  const runAnalysis = async (customQuestion?: string) => {
    ...
    const hasImage = (activeTab === 'screenshot' || activeTab === 'upload') && scamImage;
  ...
  const getReasonsList = (res: AnalysisResult): string[] => {
    ...
    return [isZh ? '来源不可信，切勿泄露个人隐私。' : 'Unverified source, never disclose personal info.'];
  };
  ...
  const handleToggleVoicePlayback = () => {
    ...
    if (level === 'scam') {
      ...
      intro = isZh ? '需谨慎，信息真实性未核实。' : 'Be Careful. Unverified message.';
    } else {
      intro = isZh ? '看似安全，属于正规官方信息。' : 'Looks Safe. Appears legitimate.';
    }
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto">
                ...
                {scamImage ? (
                  ...
                  >
                    ...
                    <p className="text-base font-bold text-[#1E293B]">
                      {isZh ? '上传短信截屏图片' : 'Upload a screenshot of the message'}
                    </p>
                    <p className="text-xs text-[#64748B] mt-1">
                      {isZh ? '支持 JPG、PNG 格式图片' : 'Supports JPG, PNG images'}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {scamImage ? (
                  ...
                  >
                    ...
                    <p className="text-base font-bold text-[#1E293B]">
                      {isZh ? '拍照或上传图片' : 'Take a photo or upload image'}
                    </p>
                    <p className="text-xs text-[#64748B] mt-1">
                      {isZh ? '支持调用手机相机拍照或相册选择' : 'Supports camera or gallery upload'}
                    </p>
                  </div>
                )}
              </div>
                ...
                {PRESET_EXAMPLES.map((item) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setActiveTab('text');
                      seniorAudio.speakText(
                        isZh ? `已载入示例：${item.labelZh}` : `Loaded ${item.labelEn}`,
                        language
                      );
                    }}
            ...
            <div className="flex-1 flex flex-col justify-between">
              ...
              <div className="max-h-[440px] h-[440px] overflow-y-auto pr-1.5 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <h2 className={`text-[22px] font-bold ${riskTitleColor} leading-tight`}>
                    {riskTitleText}
                  </h2>
                </div>
                      ...
                      <div key={idx} className="flex items-start gap-2.5">
                        <div
                          className={`w-2 h-2 rounded-full shrink-0 mt-2 ${
                            riskLevel === 'safe' ? 'bg-[#16A34A]' : 'bg-[#DC2626]'
                          }`}
                        />
                ...
                >
                  {isPlayingAudio ? (
                    ...
                    <>
                      ...
                      <span>{isZh ? '收听防骗语音建议' : 'Listen to advice'}</span>
                    </>
                  )}
                </button>
              </div>
      ...
      <div className="bg-[#F8FAFC] rounded-[20px] border border-[#E3EDF7] p-[20px] space-y-4 shadow-2xs">
        {/* Big text input with dialogue icon and integrated voice mic inside */}
        <div className="relative flex items-center">
          ...
          <input
            ...
            id="scam-custom-question-input"
            placeholder={
              isZh
                ? '针对这条信息输入您的具体疑问...'
                : 'Ask your own question about this message…'
            }
            className="w-full h-[56px] pl-12 pr-14 py-3 bg-white border border-[#E2E8F0] focus:border-[#E8590C] rounded-[14px] text-[#1E293B] font-medium text-base focus:outline-none shadow-2xs"
          />

          {/* Integrated Mic Button inside right */}
          <button
            type="button"
            onClick={handleVoiceInput}
            id="scam-mic-btn"
            className={`absolute right-2.5 p-2 rounded-xl transition cursor-pointer ${
              isRecording
                ? 'bg-rose-600 text-white animate-pulse shadow-xs'
                : 'text-[#64748B] hover:text-[#E8590C] hover:bg-[#FEF5ED]'
            }`}
        ...
        <div className="flex flex-wrap items-center gap-2 pt-0.5">
          ...
          {quickQuestions.map((q) => (
            <button
              ...
              type="button"
              onClick={() => {
                setUserQuestion(q.text);
                runAnalysis(q.text);
              }}
              className="px-3.5 py-1.5 bg-white border border-[#E8590C] text-[#E8590C] hover:bg-[#FEF5ED] active:scale-95 rounded-full text-xs sm:text-sm font-semibold transition cursor-pointer shadow-2xs"
            >
              {q.text}
            </button>
          ))}
        ...
        {/* Bottom Full-Width Orange Primary Button: "Check Scam Risk & Speak Aloud" */}
        <button
          type="button"
          onClick={() => runAnalysis()}
          disabled={isAnalyzing}
          id="scam-full-check-speak-btn"
          className="w-full h-[60px] rounded-[14px] bg-[#E8590C] hover:bg-[#cf4d07] active:scale-98 text-white font-bold text-[20px] flex items-center justify-center gap-3 transition cursor-pointer shadow-md disabled:opacity-50"
        >
          ...
          <span>
            {isAnalyzing
              ? isZh
                ? '正在全面核查并生成语音...'
                : 'Analyzing Risk & Preparing Audio...'
              : isZh
              ? '全面核查风险并语音播报'
              : 'Check Scam Risk & Speak Aloud'}
          </span>
        </button>
      </div>

      {/* 5. Bottom Disclaimer */}
      <div className="flex items-center justify-center gap-1.5 text-center pt-1 pb-1">
