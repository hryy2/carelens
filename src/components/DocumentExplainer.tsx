...
export const DocumentExplainer: React.FC<DocumentExplainerProps> = ({
  ...
  // Active result state
  const [result, setResult] = useState<AnalysisResult | null>(currentResult || null);
  ...
  const quickQuestions = isZh
    ...
    : ['What does this letter say?', 'How much do I need to pay?', 'What action do I need to take?'];
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = () => {
      ...
      seniorAudio.speakText(
        isZh ? '文件已载入，点击下方解读这封信件。' : 'Document loaded. Tap Explain to start.',
        language
      );
    };
  ...
  const handleVoiceInput = () => {
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
      seniorAudio.speakText(
        isZh ? '当前浏览器未开启语音识别，请直接打字提问。' : 'Voice input not supported in this browser.',
        language
      );
      return;
    }
    ...
    recognition.onstart = () => {
      setIsRecording(true);
      seniorAudio.speakText(isZh ? '请说，正在听...' : 'Listening to your question...', language);
    };
  ...
  const handleExplain = async (overrideQuestion?: string) => {
    ...
    const activeQ = typeof overrideQuestion === 'string' ? overrideQuestion : questionText.trim();
    setIsAnalyzing(true);
    setErrorMessage(null);

    seniorAudio.speakText(
      isZh ? '正在为您仔细解读信件，请稍候...' : 'Examining your letter now, please wait...',
      language
    );
    ...
    try {
      ...
      const msg = err.message || (isZh ? '解读遇到了问题，请再试一次。' : 'Unable to analyze. Please try again.');
      setErrorMessage(msg);
  ...
  const handleToggleSpeech = () => {
    ...
    if (!result) {
      seniorAudio.speakText(
        isZh ? '请先上传并解读信件。' : 'Please upload and explain a letter first.',
        language
      );
      return;
    }
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[28px] font-bold text-[#1E293B] leading-tight">
              {isZh ? '解读官方信件与账单' : 'Understand Official Letters & Bills'}
            </h1>
              ...
              <button
                ...
                onClick={() => {
                  if (uploadedFile) {
                    ...
                    seniorAudio.speakText(
                      isZh ? '请先上传信件照片或PDF。' : 'Please upload a letter first.',
                      language
                    );
                  }
                }}
              ...
              className={`h-[420px] w-full rounded-[12px] flex flex-col items-center justify-center relative overflow-hidden transition-all ${
                uploadedFile
                  ...
                  : `bg-[#F8FBFE] border-2 border-dashed ${
                      isDragging ? 'border-[#2B7FD4] bg-sky-50' : 'border-[#BCDCF7]'
                ...
                <div className="flex flex-col items-center justify-center p-6 text-center space-y-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-[17px] font-bold text-[#1E293B]">
                      {isZh ? '上传信件照片或PDF' : 'Upload a photo of your letter'}
                    </p>
                    <p className="text-[13px] text-[#64748B]">
                      {isZh ? '支持 JPG、PNG 图片或 PDF 文件' : 'Supports JPG, PNG images or PDF files'}
                    </p>
                  </div>
                </div>
              )}
              ...
              {viewState === 'question' && submittedQuestion ? (
                ...
                <>
                  ...
                  >
                    ...
                    <span>{isZh ? '查看完整解读' : 'View full explanation'}</span>
                  </button>
                </>
              ) : (
                /* 状态A: 顶部标题 "Here's what this letter means:" */
                <h3 className="text-[17px] font-bold text-[#1E293B]">
                  {isZh ? '这封信的意思是：' : "Here's what this letter means:"}
                </h3>
              )}
            </div>
            ...
            <div className="max-h-[420px] h-[420px] overflow-y-auto pr-1">
              {isAnalyzing ? (
                ...
                <div className="h-full flex flex-col items-center justify-center p-8 text-center space-y-4">
                  ...
                  <div className="space-y-1">
                    <p className="text-[17px] font-bold text-[#1E293B]">
                      {isZh ? '正在为您仔细解读信件...' : 'Analyzing this letter for you...'}
                    </p>
                ...
                <div className="space-y-3.5">
                  ...
                  <div className="bg-slate-50/80 rounded-[14px] p-4 border border-[#E3EDF7] space-y-2.5">
                    ...
                    {result?.detailedExplanation && result.detailedExplanation !== result.summary && (
                ...
                <div className="bg-[#F1F7FE] rounded-[16px] p-3.5 sm:p-4 space-y-2.5">
                  ...
                  <div className="flex items-start gap-3 bg-white rounded-[12px] p-3 border border-[#E3EDF7]">
                    ...
                    <div className="flex-1">
                      <div className="text-[14px] font-bold text-[#1E293B]">
                        {isZh ? 'Type of letter — 信件类型与发件方' : 'Type of letter'}
                      </div>
                      <div className="text-[15px] text-[#475569] leading-[1.5] mt-0.5">
                        {result ? (
                          ...
                          <span className="text-[#94A3B8]">
                            {isZh ? '上传并解读后将显示发件方与信件类型' : 'Sender and document category'}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-start gap-3 bg-white rounded-[12px] p-3 border border-[#E3EDF7]">
                    ...
                    <div className="flex-1">
                      <div className="text-[14px] font-bold text-[#1E293B]">
                        {isZh ? 'What it says — 信件主要内容说明' : 'What it says'}
                      </div>
                      <div className="text-[15px] text-[#475569] leading-[1.5] mt-0.5">
                        {result?.summary ? (
                          result.summary
                          ...
                          <span className="text-[#94A3B8]">
                            {isZh ? '上传后为您用大白话提炼信里说了什么' : 'Summary of what the letter is about in simple terms'}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-start gap-3 bg-white rounded-[12px] p-3 border border-[#E3EDF7]">
                    ...
                    <div className="flex-1">
                      <div className="text-[14px] font-bold text-[#1E293B]">
                        {isZh ? 'What you need to do — 需要采取的行动' : 'What you need to do'}
                      </div>
                      <div className="text-[15px] text-[#475569] leading-[1.5] mt-0.5">
                        {result?.actionSteps && result.actionSteps.length > 0 ? (
                          <ul className="space-y-1">
                            {result.actionSteps.map((step, idx) => (
                              <li key={idx} className="flex items-start gap-1.5">
                                <span className="text-[#2B7FD4] font-bold shrink-0 mt-0.5">•</span>
                                <span>{step}</span>
                              </li>
                            ))}
                          </ul>
                        ) : result ? (
                          <span>{isZh ? '纯通知信件，无需采取任何行动。' : 'No action required.'}</span>
                        ) : (
                          <span className="text-[#94A3B8]">
                            {isZh ? '明确告诉您是否需要交费、回信或办理手续' : 'Clear steps you need to take, or confirmation if none needed'}
                          </span>
                        )}
                      </div>
                    </div>
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-1">
                            <span>
                              ...
                              <strong className="text-[#1E293B]">
                                {result.keyDetails?.keyDateOrTime || (isZh ? '无特定截止日期' : 'None')}
                              </strong>
                            </span>
                            <span>
                              {isZh ? '金额：' : 'Amount: '}
                              <strong className="text-[#1E293B]">
                                {result.keyDetails?.amountOrCost || (isZh ? '无需付款' : 'None')}
                              </strong>
                            </span>
                          </div>
                        ) : (
                          <span className="text-[#94A3B8]">
                            {isZh ? '自动标出截止日期、还款金额或到账周期' : 'Important deadlines and amounts to pay or receive'}
                          </span>
                        )}
                      </div>
                    </div>
            ...
            >
              {isSpeaking ? (
                ...
                <>
                  ...
                  <span>{isZh ? '听取完整语音讲解' : 'Listen to full explanation'}</span>
                </>
              )}
            </button>
          </div>
      ...
      {/* ========================================================= */}
      <div className="bg-white rounded-[20px] p-5 sm:p-6 border border-[#E3EDF7] shadow-sm space-y-3.5">
        {/* 一个大的文字输入框，左侧带对话图标，右侧内部带麦克风图标按钮（语音输入），高度约 56px */}
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full h-[56px] pl-12 pr-12 bg-white rounded-[14px] border border-[#E2E8F0] focus:border-[#2B7FD4] text-[#1E293B] text-[15px] sm:text-[16px] font-medium placeholder:text-[#94A3B8] outline-none transition shadow-2xs"
          />
          ...
          <button
            ...
            className={`absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-[10px] flex items-center justify-center transition cursor-pointer ${
              isRecording
                ? 'bg-rose-500 text-white animate-pulse'
                : 'text-[#64748B] hover:text-[#2B7FD4] hover:bg-[#F1F7FE]'
            }`}
          >
        ...
        {/* 下方一行："Quick questions:"标签 + 3 个快捷问题胶囊按钮（白底、蓝色描边、蓝字） */}
        <div className="flex flex-wrap items-center gap-2 pt-0.5">
          <span className="text-[13px] text-[#64748B] font-semibold mr-1">
          ...
          {quickQuestions.map((q, idx) => (
            <button
              ...
              className="bg-white border border-[#2B7FD4] text-[#2B7FD4] hover:bg-[#F1F7FE] active:scale-95 text-[13px] font-semibold px-3.5 py-1.5 rounded-full transition cursor-pointer shadow-2xs"
            >
              {q}
            </button>
          ))}
        ...
        {/* 最下方：全宽蓝色主按钮"Explain This Letter"（高度 60px、圆角 14px、带✨图标、字号 20px 粗体） */}
        <button
          type="button"
          onClick={() => handleExplain()}
          id="btn-explain-letter-main"
          disabled={isAnalyzing}
          className="w-full h-[60px] bg-[#2B7FD4] hover:bg-[#236bb3] active:scale-98 disabled:opacity-60 text-white rounded-[14px] text-[20px] font-bold flex items-center justify-center gap-2.5 shadow-sm transition cursor-pointer mt-1"
        >
          {isAnalyzing ? (
            <>
              <Loader2 className="w-6 h-6 animate-spin" />
              <span>{isZh ? '正在为您解读中...' : 'Analyzing Letter...'}</span>
            </>
          ) : (
            <>
              <Sparkles className="w-6 h-6 text-amber-300" />
              <span>{isZh ? '解读这封信件' : 'Explain This Letter'}</span>
            </>
          )}
        </button>
      </div>
      ...
      {/* 四、底部免责提示 */}
      {/* ========================================================= */}
      <div className="pt-2 pb-1 flex items-center justify-center text-center">
        <p className="text-[13px] text-[#94A3B8] font-medium flex items-center justify-center gap-1.5">
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div className="flex items-center gap-2 font-bold text-slate-800 text-lg">
                <FileText className="w-5 h-5 text-[#2B7FD4]" />
                <span>{uploadedFile.name || (isZh ? '信件大图预览' : 'Document Preview')}</span>
              </div>
              <button
                type="button"
                onClick={() => setIsZoomModalOpen(false)}
